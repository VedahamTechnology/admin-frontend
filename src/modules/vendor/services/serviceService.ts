...
const API = `${vendorApiBaseUrl}/services`;
...
export const getVendorServices = async (): Promise<GetMyServicesResponse> => {
  const res = await axios.get<GetMyServicesResponse>(API, getVendorRequestConfig());
  return res.data;
};
...
export const createService = async (
  ...
  const res = await axios.post<CreateServiceResponse>(API, data, getVendorRequestConfig());
  return res.data;
};
...
export const deleteService = async (id: string): Promise<DeleteServiceResponse> => {
  const res = await axios.delete<DeleteServiceResponse>(
    `${API}/${id}`,
    getVendorRequestConfig()
  );
